...
for qid, value in qid_to_loss.items():
    ...
    losschange = np.abs(qid_to_loss[qid][0] - qid_to_loss[qid][1])
    all_loss_changes.append(losschange)
pcc_alpha_vs_losschange = stats.pearsonr(all_alphas, all_loss_changes)
print("PCC alpha oracle vs. loss change: {:.3f}".format(pcc_alpha_vs_losschange.statistic))